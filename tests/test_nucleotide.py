...
from query import NucleotideSearch
from route.summary.nucleotide import get_run_route, run_cache

search = NucleotideSearch()

def test_run_summary():
    result = search.get_run_properties('ERR2756788')
    assert result == nsra(run_id='ERR2756788', read_length=150, genome='cov3ma', version='200818', date='200817-21:05')

    result = search.get_run_families('ERR2756788')
    assert len(result) == 12
    assert result[0] == nfamily(run_id='ERR2756788', family_name='AMR', coverage_bins='mooaoooUmmmmmooooaaoawwwm', score=100, percent_identity=97, depth=214.8, n_reads=1382, n_global_reads=301, length=1000)

    result = search.get_run_sequences('ERR2756788')
    assert len(result) == 349
    assert result[0] == nsequence(run_id='ERR2756788', family_name='Coronaviridae', genbank_id='HQ728485.1', coverage_bins=':a..__uu__a_________o____', score=28, percent_identity=85, depth=0.7, n_reads=119, n_global_reads=98, length=13015, genbank_name='Miniopterus bat coronavirus/Kenya/KY33/2006 polyprotein (ORF1ab) gene, partial cds; and spike protein (S), ORF3 protein (ORF3), envelope protein (E), membrane protein (M), nucleocapsid protein (N), and hypothetical protein ORFx (ORFx) genes, complete cds')
...
def test_download_family():
    contents = search.get_matches_file(family='Coronaviridae', scoreMin=100)
    with open('tests/SerratusMatches-family.csv') as f:
        assert contents == f.read()


def test_download_genbank():
    contents = search.get_matches_file(genbank='EU769558.1', scoreMax=50)
    with open('tests/SerratusMatches-genbank.csv') as f:
        assert contents == f.read()


def test_paginate_family():
    pagination = search.get_matches_paginated(family='Coronaviridae', scoreMin=100)
    ...
    pagination = search.get_matches_paginated(family='Coronaviridae', scoreMin=100, perPage=3)
    assert len(pagination.items) == 3


def test_paginate_genbank():
    pagination = search.get_matches_paginated(genbank='EU769558.1', scoreMax=50)
    assert len(pagination.items) == 20
    ...
    pagination = search.get_matches_paginated(genbank='EU769558.1', scoreMax=50, perPage=3)
    assert len(pagination.items) == 3


def test_counts():
    counts = search.get_counts(family='Coronaviridae')
    assert len(counts) == 1320
    assert counts[10] == {'score': 1, 'percent_identity': 97, 'count': 43356}

    counts = search.get_counts(genbank='EU769558.1')
...
def test_list():
    values_list = search.get_list('family')
    assert len(values_list) == 46

    values_list = search.get_list('genbank')
